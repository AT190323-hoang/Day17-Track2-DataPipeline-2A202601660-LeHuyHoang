...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    ghi đè đúng hàng đó thay vì tạo thêm hàng mới.
    """
    con.executemany(
        f"""
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            # at-least-once: ghi dữ liệu TRƯỚC, commit offset SAU. Nếu chết
            # giữa hai bước, batch đã ghi nhưng offset chưa dịch -> lần khởi
            # động lại đọc lại đúng batch đó (replay), không mất dữ liệu.
            # write_batch() idempotent (ON CONFLICT) nên replay không trùng.
